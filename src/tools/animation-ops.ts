...
const TOOL_NAMES = ['animation', 'animation_track', 'animation_keyframe', 'animation_curve', 'animation_blend'] as const;
...
const LOOP_MODES = ['none', 'linear', 'pingpong'] as const;
...
// ─── New Tool GDScript Generators (P2) ────────────────────────────────────
...
function valueToGd(v: unknown, trackType?: string): string {
...
function argsToGd(args?: unknown[]): string {
  if (!args || args.length === 0) return '[]';
  return `[${args.map(a => valueToGd(a)).join(', ')}]`;
}

function animErrorMapper(errorMsg: string): string {
