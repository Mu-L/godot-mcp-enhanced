...
- **ui_pixel_verify(像素终验)**：对已构建场景（\`scene_path\` 必填，指 \`ui_import_prototype\` persist 产物）窗口模式截图，对每个 bg 节点采样中心+四角内缩点（内缩 \`min(borderRadius+border.width, 短边/2−2)\`，角点坐标按像素格半开区间右/下缘 \`−1\`），与目标色 RGB 欧氏距离判定（中心容差 20、角点 60，0-255 空间；实测校准零底噪零偏移，13 采样点 distance=0）。**定位是终验而非迭代反馈**——几何 layout_verify + style_verify 全绿后才跑一次（每次 capture 窗口模式弹窗 + 秒级耗时）。Windows 窗口模式专属（headless=dummy renderer 截图空白；Linux headless 2D CanvasItem 亦空白，BLANK 拦截为 stdout 提示 + PNG 全图均匀色双条件）。采样跳过两类：半透明 bg（alpha<1，合成后采样色≠bg_color）与 ProgressBar 系（type/value/fill 任一——fill 与百分比文字覆盖 bg 渲染面，无可采样纯色区，bg 槽依赖 style_verify）；带 text 节点跳过中心点（居中排版必踩文字，仅采四角）。采样基准=输入视口 rect，挂载父须原点对齐（同 layout_verify 约束）；flow 子节点位置偏差由 flow_verify 先收敛；PNG 中间产物临时落 \`.godot/\` 且失败路径也清理。
