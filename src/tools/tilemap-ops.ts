 ...
 * 转义:纯字面量内插走 escapeForGdLiteral(债务批约定,% 不双写;scenePreamble
 * 由 PR#36 引入,集成时对齐)。
 */
...
export async function handleTool(
  ...
  try {
    ...
    // 越界(../ 逃逸/白名单外)→ INVALID_PARAMS(resolveWithinRoot 抛错映射,同 ui 工具
    ...
    if (args.scene_path !== undefined && args.scene_path !== null) {
      try {
        ...
        return opsErrorResult('INVALID_PARAMS', `scene_path 非法: ${getErrorMessage(err)}`);
      }
    }
    let script: string;
