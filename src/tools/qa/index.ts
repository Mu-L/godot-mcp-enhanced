...
export function getToolDefinitions(): Tool[] {
  return [{
    name: 'qa',
    description: 'QA 测试套件编排：结构化测试规范 → 自动安装 bridge → 运行游戏 → 逐步执行'
      + '（输入/等待/断言/确定性 playtest/截图证据）→ 聚合报告 + 回归 diff（夜间跑批用）。'
      + '步骤 type：input(send_key/mouse_click/mouse_move/text/touch/drag)、wait(wait_for_node/'
      + 'wait_for_property)、wait_frames、freeze/unfreeze、step_until(结构化条件)、snapshot/'
    ...
    inputSchema: {
      ...
      properties: {
        ...
        // run
        spec: { type: 'object', description: 'run: inline 套件 spec 对象（结构见 description；步骤带 label 便于 diff 对齐）' },
        spec_path: { type: 'string', description: 'run: spec 文件路径（.json 或含 ```qa-spec 围栏的 .md），大套件建议用文件避免 token 截断' },
