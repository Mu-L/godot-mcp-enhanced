...
> **版本口径改判(patch→minor)**:0.32.12-0.32.21 的 P0-P10 十一个批次均为新增功能(新工具 dap/uid/translation、新 action sync_state/send_input_sequence/network_conditioner 等),按语义化版本应为 minor 级;此前一路 patch 递增是「规则模板硬门禁」机械触发 `npm version patch` 的惯性,未按变更性质判级。自本版起归位正确口径——本版含 P0-P10 之后的全仓审查修复,且该批修复伴随大量行为语义修正,以 0.33.0 标记这条 minor 边界。

- **审查方法与范围**:v0.32.21 全部功能模块(46 工具/271 action)——4 个分域只读审查代理(dap / mcp_bridge.gd / TS 执行安全链 / bridge TS+生命周期)逐行实测 + 主审横向机械扫描(TODO/空 catch/定时器/动态正则/spawn 注入面) + 业务冒烟(smoke 72 用例/CLI doctor)。
- **S-B1(BLOCKING)project_replace 写 .gd 完全绕过沙箱扫描**:批量落盘只过 pluginSelfPathGuard,违反自家 `src/tools/script.ts` "全仓所有写 .gd 落盘前必须过此扫描"约束(replace 注入 OS.execute → run_project 即执行)——批量原子检查块对每个 .gd 内容过 `scanScriptSandboxOrThrow`,命中整批拒;回归测试锁定(test/script-project-replace.test.js 注入用例 6 passed)。
- **G-B1(BLOCKING)step_until 完成判定嵌在 for cond 循环体内**(src/scripts/mcp_bridge.gd):① AND 语义破坏——conditions=[A,B] 时 A 满足即完成、B 未评估,predicate_met 谎报全满足;② 同 su_idx 双 append → 消费循环倒序双 remove:单 pending 时空数组越界**中断 _process 当帧**(monitor/watch/custom tick 全跳过),多 pending 时**误删相邻条目**(响应永久丢失+错误 refreeze)。判定移出循环,多条件 AND 语义 e2e 回归锁定。
- **dap 域(4 IMPORTANT+2 MINOR)**:D-I1 output 收集用 messages.length 做游标与 _trimMessages 的 200 上限**数学互斥**(稳态 push 即 shift,slice 恒空——游戏 print 超 200 条后 output action 永久静默失效)→ 消息附单调 `__mcpSeq` 过滤窗口+所有输出出口剥离内部字段;D-I2 socket FIN 断连后残留 initialized=true,带前置的 action 经旧 session 校验通过但重建未握手新连接 → `rebuilt` 标记+立即要求 re-run initialize;D-I3 断点簿记 key 无路径规范化(`D:/a.gd` 与 `D:\a.gd` 生成多份独立簿记,幽灵断点/上限绕过)→ `_normalizeSourceKey`(resolve+Windows 小写+正斜杠统一);D-I4 "超时路径带 messages"测试虚假覆盖(实际命中 endpoint 守卫错误)→ 重写真 initialize 建会话走真超时+显式断言+close 悬挂 finally disconnect;D-M5 capabilities 补 `_sanitizeValue`;D-M6 buffer 上限 +4KB header 余量(恰好 1MB 合法帧先被误杀)。
- **bridge TS+生命周期(2 IMPORTANT)**:B-I1 sync_state 丢弃 GD 侧 truncated 截断标志(P10 审查 N-2 只修了一半——256+ 节点同截断→in_sync 假阳性)→ SyncSnapshot 加 truncated 字段,snapshot 透传+compare 标注 `unreliable: true`;B-I2 **game_wait 单次 probe 超时烧掉整个等待预算**——probe 超时=interval×2(默认 400ms),主线程卡顿>400ms(场景加载/物理重帧常见)即整体失败,30s 总预算只耗 0.4s,错误建议"加大 timeout"无效 → pollWaitCondition 对 probe throw 容错继续轮询(同日 p7-e2e 400ms 假失败的产品侧根因,测试参数加固成纵深双保险)。
- **mcp_bridge.gd + websocket_server.gd(7 IMPORTANT+1 MINOR)**:G-I1 step_until 校验失败仍抢注 owner+残留过期 paused 原值(赋值随开窗副作用移到全部校验之后);G-I2 find_nodes near_node 锚点缺 player 档可观察性检查(private/级联隐藏节点可当锚点差分追踪位置轨迹,穿透 P7 存在性不泄露承诺)→ 锚点补 `_observable_in_player` 检查;G-I3 collect_state 树遍历无 freed 守卫(用户 `_mcp_state()` 内 `.free()` 致已收集 200+ 节点状态静默全丢)→ pop 后 is_instance_valid 守卫跳过;G-I4 未认证 peer 空行保活 DoS(每秒发空行无限续命,5 连接占满 MAX_PEERS 合法客户端永久被拒,无需 secret)→ 未认证空行直接断连;G-I5 websocket_server 顶层标量 JSON("123"/1.5/"x" 经 parse_string 返回 int/float/String 无 has() 方法)运行时错误中断 _process——**无需认证持续发送即实质瘫痪 editor WS 服务** → 改 `not (parsed is Dictionary)`(对齐 bridge 侧既有正确写法);G-I6 custom 命令协程 quiesce 失真(callable.call() 在首个 await 挂起即返回,active_calls 立即归零,热重载释放仍在执行的协程宿主)→ get_class() 字符串比较检测协程(`is GDScriptFunctionState` 类型检查在 4.x Parse Error 挂掉 autoload——实测踩坑);G-M2 playtest.step 补 owner 互斥(对齐 seed 同款)。
- **TS 执行安全链(S-I2+S-M1/S-M3/S-M4)**:S-I2 **cmdline 标记窃取链拦截**——`OS.get_cmdline_args()` 取回脚本路径+变量首参 FileAccess.open 读包装后脚本正文恢复随机 marker(两条 FileAccess 模式均不匹配变量首参的黑名单盲区)→ DANGEROUS_PATTERNS 增补;S-M1 cleanupOldSessions 正则 `/-(\d+)-$/` 与 mkdtemp 随机后缀永不匹配(A-02 文件名时间戳解析从未生效恒走 mtime 兜底)→ 前缀锚定引用 TMP_PREFIX;S-M3 execute_gdscript 的 outputs.value/raw_output 补 `maybeWrapUntrusted` 信封包装(对齐 workflow 通道);S-M4 wrapUntrusted kind/source 属性 `"` → `&quot;` 转义(source 来自用户 _mcp_output key 攻击者可控,含 " 逃逸信封开标签属性域)。
- **S-I1 处置记录(scrub 修复撤销——契约 vs 漏洞的判定教训)**:初判 IMPORTANT"随机输出标记经 replaceAll 暴露给用户代码",实测**撤销**——固定常量 `___MCP_RESULT___`/`___MCP_ERROR___` 是 C-09 的公开协议契约(ui-layout 等 full-class 测试模板与生成器统一"写常量、server replaceAll 换随机 marker"),scrub 破坏契约致全量 41 用例结构化输出解析全断(二分定位+取证 log 破案);威胁模型重审:单脚本内输出控制权本属脚本作者(marker 防跨执行/重放伪造非当次),execute 通道注入防御由 untrusted 信封承担。降级为设计声明,保留处置注释(src/gdscript-executor.ts executeGdscript 入口)。**教训:改"看似防御"的安全逻辑前先 grep 全仓对该机制的依赖面**。
- **备案未修(10 项,理由见审查报告§三)**:GD I-7 响应阻塞 put_data 慢消费者冻结(架构级下批专项)/`_observable_in_player` 64 层截断 fail-open/playtest.snapshot 深度不限/file-guard 覆盖面声明/dap allow_remote_hosts 联动/`_syncSnapshots` 无上限(有 clear 兜底)/process-state close 不清(存量 23 setter 长线项)/helpers barrel 逾期等。
- **流程盲区新发现(2 项)**:check:gdscript 对 autoload parse error 漏报(`GDScriptFunctionState` 事故 errors=0,手起游戏才暴露,需另开小项);e2e fixture 的 mcp_bridge.gd drift 保守策略放大测试陈旧(gd-symmetry 用旧副本跑,GD-B1 修复"看似无效")。
- **验证**:`npm run lint` 0 错;`npm run build` 过;`npm run check:gdscript` errors=0;分域复验 dap.test 20/20(含重写 DAP-o/新增 DAP-s)、script-project-replace 6/6、game-bridge-wait 8/8(含 2 个 probe 容错)、p10-unit 15/15(含 TS-b3)、gd-symmetry L2 5/5(含 GD-B1 多条件 AND);全量 `npm test` 6348 过/1 flaky 超时(ui-layout-integration 嵌套 rect 用例,单文件复验 7/7 绿——全量并行机器负载所致非回归)/80 skipped。

## [0.32.21] - 2026-09-12

### Added — P10 批:多人状态同步 sync_state(masteryee 移植裁剪)(42 仓尽调 4.2.1,docs/reviews/2026-09-12-p10-batch.md)

- **P10 sync_state**(game 工具新 action,masteryee sync_state 移植裁剪):**快照/比对两段式原语,不做进程编排**——清单"enhanced 已有多实例管理"的假设不成立(InstanceManager 是 MCP server 实例注册表,非游戏进程编排),多游戏实例由用户/agent 起在各端口(bridge 端口避让已有),借连接切换打多个快照后比对。设计与清单偏离声明。
- **GD 侧 collect_state 命令**(src/scripts/mcp_bridge.gd):收集约定——节点实现 `_mcp_state() -> Dictionary` 即被收集(游戏侧声明**离散状态**;浮点位置类建议游戏侧量化);返回非 Dictionary 记 __error__;可选 group(如 mcp_watch)组内无 _mcp_state 成员记 `{"__present__": true}` 存在性标记(节点集增删可测);嵌套 Object 递归降级 str() 不炸 JSON;上限 256 节点/深度 8;game_time_ms 时间戳。
- **TS 侧 sync_state**(src/tools/game-bridge.ts):sub_action=snapshot(label/group→collect_state 存内存快照)/compare(label_a/label_b/tolerance)/list/clear;`compareStates` 纯函数导出(递归比对:数值 |a-b|<=tolerance 容差/dict 逐键/数组逐项+长度/类型不匹配严格不等;键级 diff 输出 missing_in_b/missing_in_a/diffs)。action 计数 270→271(sync_state=read:只收集+本地比对不改游戏状态)。
- **浮点容差是关键设计(masteryee 亲读坑的修复)**:原版 digest 比对不做 hash 且默认属性含浮点 position——host/client 各自物理步进后位置不逐位相等,dict 严格相等在真实多人游戏**永远 false**,实际依赖目标游戏实现 _mcp_state 返回离散状态。enhanced 侧:容差参数(tolerance 默认 0.0001)进比对语义,离散状态约定进收集端——两端都兜。
- **测试**:test/p10-unit.test.ts 11 用例(GD 契约 4:命令注册/_mcp_state 约定/上限+降级/时间戳;TS 契约 1;compareStates 纯函数 6:一致/浮点容差与严格对照/节点集差异/键级 diff+__present__/嵌套数组与容差/类型不匹配)+ p3-e2e 追加 SYNC 用例(真起游戏:snapshot→GDA_CALLABLE toggle_p10_flag 改状态→snapshot→compare 出 flag/ticks 两键 diff→自身对照 in_sync→list/clear;fixture main.gd 实现 _mcp_state 约定含 Vector2 pos 验证浮点序列化)。
- **分发规则双副本**:bridge.md 新增「多人状态同步」段(收集约定/group 存在性/快照流/浮点容差/比对输出/典型用法)(rule-templates.ts + .claude/rules,触发 bump 硬门禁——本批定版 0.32.21)。
- **独立审查与清偿(B-1/N-1..N-5 + 2 旧锚,docs/reviews/2026-09-12-p10-batch.md)**:code-reviewer 判定 BLOCKING——**B-1(置信 88)容差对位置类完全失效**:_state_safe 放行 Vector2 → JSON.stringify 退化 "(10.000001, 20)" 字符串(仓库 send_drag 先例自证的退化),TS 容差只对 number 生效 → host/client 位置比对永远 false,masteryee 坑在核心场景原样复现;且 fixture 常量 Vector2 与实现盲区重合(两侧字符串相等恰好过测,"微差容差判等"从未被验证);同根因:INF→1e99999(同值 Infinity 恒 diff)/NAN→null(值漂移)。清偿三面:GD 几何类型走既有 _jsonify 转 {x,y,z} dict(分量级容差)+float is_finite 检查;TS valuesEqual 数值分支 va===vb 严格短路(同值 Infinity 不经容差算术);e2e 锚 toggle 顺带微移 pos(+0.00001 容差内)真实验证微差 Vector2 不误报。N-1:collect_state 不经 P7 观察层按声明面模式补边界声明(双副本)。N-2:256 截断加 truncated 标记(防两侧同截断 in_sync 假阴性)。N-3:risk 定级注释清理(read 纯读通道 vs custom_command write 的差异说清)。N-4:label 进程全局提示。N-5:审查文档落盘闭环。全量另抓 2 旧锚(game-bridge enum 锁清单+p4-batch SLIM-a 6500→7800 正当增量校准)。清偿过程自曝工具坑:Git Bash heredoc 传中文 pattern 给 python 终端转码失真(三轮替换 assert 过但内容未变的灵异根因),改 Write 工具落 .py 文件后一次成功——教训登 memory。

## [0.32.20] - 2026-09-12

### Added — P9 批:DAP 断点调试工具(LuoxuanLove 移植,TS 直连 editor DAP server)(42 仓尽调 5.2.2/行动清单 #8,docs/reviews/2026-09-12-p9-batch.md)

- **P9 dap 工具**(src/tools/dap.ts,~700 行 TS,新增第 46 个顶层工具):MCP server 作为 **DAP client** TCP 直连 Godot editor 自带 DAP server(默认 127.0.0.1:6006)——引擎官方调试协议,驱动 editor 已有调试端点,比对标竞品"按图标找调试器按钮"的面板 hack 稳。18 action:status/get_settings/set_settings/initialize/launch/attach/configuration_done/disconnect/terminate/threads/set_breakpoint/remove_breakpoint/list_breakpoints/pause/continue/step_over/stack_trace/output。
- **架构决策(设计偏离声明)**:原实现(LuoxuanLove executor.gd 791 行)跑在 GD editor 插件内(StreamPeerTCP + await process_frame);enhanced 改 **TS 侧 node:net 直连**——①无 GD 改动(不增加 editor 插件分发/版本兼容负担);②DAP 场景本就要求 editor 运行中+调试服务器开启,前置一致;③Node 的 TCP/async/帧解码是标准操作,frame 编解码从 GD 最重部分缩到 ~40 行。与既有 debug 工具(editor 层 EditorDebuggerPlugin)并存互补,规则文档写明选择建议(断点/单步优先 dap)。
- **保留(LuoxuanLove 同款防线)**:三段会话状态机(initialized→launched→configured,乱序拒+端点变更守卫);断点簿记自存+setBreakpoints 全量重发(DAP 全量语义,上限 512 sources × 256 lines);SENSITIVE_KEYS/VALUE_MARKERS 双层清洗(token/password/authorization/bearer 等键值→[redacted],防调试输出泄漏 secret);loopback 强制(非 127.0.0.1/localhost/::1 需 set_settings allow_remote_hosts 显式开);超时 30s cap/会话 8/消息 200/buffer+frame 1MB 上限族。
- **SSOT 审计器首战抓漂移**:check:ssot-params 上线后第一批新代码即抓出 dap handler 读取 args.path/args.disconnect/args.terminateDebuggee 未声明——path 别名删除(新工具无历史包袱)、disconnect/client_id/client_name/adapter_id 补 schema、驼峰回退删除。防线自证有效。
- **已知边界(诚实声明)**:只覆盖 GDScript(C# 断点需 .NET debugger,引擎 DAP scope);attach 对 Godot DAP 支持有限(协议层透传);stack_trace 需暂停在断点;e2e 未覆盖真 editor(单测 mock DAP server 走真实 Content-Length 帧协议覆盖协议层/状态机/清洗/上限/loopback/超时;真 editor 路径需手动开 Editor Settings → Network → Debug Adapter,规则文档写明步骤)。
- **工具计数全链同步(45→46/252→270)**:capability-matrix 重建 + tool-count 24 处(README/README.en/manifest/server.json/distribution/migration/rule-templates/core.md)+ risk 子数对齐真实分布(read 132+write 112+destructive 10+process 16);**修复两批累积的版本表历史行误伤**(P8 批批量替换曾把历史行 248→252,本批脚本又 252→270、45→46——已按各批写入时真相恢复 v0.32.13-0.32.19 行计数;教训:批量计数替换绝不碰版本表历史行)。
- **分发规则双副本**:core.md 新增「dap — DAP 断点调试」小节(前置开启步骤/会话流/断点语义/安全/限制)+ check-rules-content-sync 端口白名单扩 DAP 6006(rule-templates.ts + .claude/rules,触发 bump 硬门禁——本批定版 0.32.20)。
- **测试**:test/dap.test.ts 15 用例(mock DAP server node:net 假 server 真实帧协议:工具定义+风险登记 18/initialize 握手+capabilities/状态机前置校验/断点簿记全量重发/256 上限/敏感清洗/loopback 门禁/超时/端点变更守卫/disconnect 清会话/res:// 转绝对路径/set_settings 白名单/output 收集+清洗/timeout 30s cap/连不上报错+开启指引)。
- **独立审查与清偿(N-1..N-5 + 全量测试 3 处注册面,docs/reviews/2026-09-12-p9-batch.md)**:code-reviewer 判定 SHIPPED WITH NITS(0 Blocking)——**N-1(置信 90,建议升格)include_raw 在 17/18 action 无效**(schema 承诺附带原始 DAP request/messages,实际仅 output 实现;失败路径 _args 收而不用——调试工具自身排障路径失效;SSOT 审计器盲区实证:键被引用一次即通过对账,不校验行为承诺全 action 生效);清偿:成功+失败路径补 include_raw+回归 DAP-o。N-2:set_settings 同批次 {host:远程, allow_remote_hosts:true} 顺序回归(TS 逐键循环因旧值先拒 host;还原 gd 预处理顺序——循环前先应用 allow_remote_hosts)。N-3:push 模型 pump 检出的 buffer/frame 超限错误静默降级为 dap_timeout——加 pendingError 通道,_readMessages 优先报出+回归 DAP-q(mock 巨型 Content-Length header)。N-4:同 session 并发首连 socket 覆盖泄漏——_connectInflight Promise 复用(参照 bridge-client 串行化)+回归 DAP-r。N-5:budget totalSum warn 95→105KB 校准(P8+P9 正当增量 ~103.9KB 越旧线,注释记录;同步 p4-batch/check-token-budget.test 两处阈值锁)。全量测试另抓 3 处:risk-coverage GUARDED_KEYS 加 dap(defects 基线 64→66/p8-unit 计数文案放宽)。清偿后 dap 单测 19/19+门禁全绿。
- **版本表历史行误伤修复(本批自曝+P8 遗留)**:P8 批批量计数替换曾把 README 版本表历史行 248→252,本批脚本又 252→270/45→46——已按各批写入时真相恢复 v0.32.13-0.32.19 行(45 工具/248-252);教训沉淀:批量计数替换绝不碰版本表历史行,新工具计数只改顶层当前描述行。

## [0.32.19] - 2026-09-12

### Added — P8 批:mcp_commands 热加载状态机(LuoxuanLove 移植)+ 参数 SSOT 双防线(regiellis 移植)(42 仓尽调组1 #12/组3 #2,docs/reviews/2026-09-11-p8-batch.md)

- **P8-1 自定义命令热加载状态机**(LuoxuanLove user executor.gd 497 行移植,src/scripts/mcp_bridge.gd):游戏运行中改/增/删 `res://mcp_commands/*.gd` 自动重载,无需重启。slot 模型 `{instance,commands,version,state,active_calls,pending_reload,removed_pending,last_mtime,last_error}`;mtime 对比 → 300ms debounce tick(`_process` 限频,无目录秒回);**quiesce 语义**——命令调用中(active_calls>0)绝不换实例,重载/卸载推迟到调用归零(waiting_quiesce);**失败回滚**——契约违反(非 Node/缺 get_commands/非 Dictionary)时 slot 进 reload_failed + last_error,旧实例与旧命令原样继续服务;删除文件调用中 removed_pending 等归零再卸载;重名冲突路径字典序靠前者赢(后者 reload_failed 清空;索引按 sort 顺序全量重建与注册时间无关——审查 N-3 措辞修正);内建诊断 `custom.list` 返回 slot 状态快照;env `GODOT_MCP_BRIDGE_CUSTOM_HOT_RELOAD=0` 关闭热重载(退化启动加载一次)。
- **P8-1 关键设计:GDScript.new() 独立加载,不走 ResourceLoader**。e2e HOT-c 二轮实测抓出的引擎行为:同路径 `ResourceLoader.load` 即使 `CACHE_MODE_IGNORE` 也会**就地替换 ResourceCache 共享 GDScript 资源**——旧实例的方法表随之消失,回滚保留的旧 Callable `is_valid()=false`(诊断打印定位:old_inst_valid=true 但 callable_valid=false,即"实例活着但方法没了")。改用 FileAccess 读源码 + `GDScript.new()` 独立对象加载:坏文件加载失败即丢弃,旧脚本资源从未被触碰,回滚退化为"什么都不做",天然有效且状态机更简。
- **P8-1 已知引擎限制(诚实边界)**:运行中写入**语法坏** .gd 触发 Godot 4.6 Script Debugger REPL(debug> 提示符停等 stdin,主循环挂死,bridge 不可防;GDScript.reload() 预检同样触发,已试并放弃)——语法错误 IDE 侧先消灭;bridge 防线覆盖"合法语法但违反契约"的坏文件(e2e HOT-c 用 extends RefCounted 验证)。规则文档双副本同步声明。
- **P8-2 unknown-param 拒绝 + did-you-mean**(regiellis 移植,src/core/args-validator.ts):所有工具顶层未声明参数一律拒绝(早期 additionalProperties 静默容忍语义反转——schema 删键曾等于"运行时仍默默读",现在是"立刻报错",防 schema 与行为漂移);拼写接近返回 did-you-mean 建议(Levenshtein 相似度 > 0.4,对齐 GD String.similarity 语义;如 alpah → 想传 alpha?)。仅查顶层(schema.properties 非空才查,无 properties 的自由 dict 不查;嵌套层不查)。
- **P8-3 SSOT 审计器**(scripts/check-ssot-params.mjs,CI 阻断 + package.json `check:ssot-params`):文件自证归属(扫 TOOL_META 顶层键 + name 字面量,不信 import 推断——第一版正则不匹配 namespace import 曾全体跳过零漂移假绿)+ 文件级并集对账(handler args.get()/属性访问的键 ⊆ 声明键 ∪ 白名单)+ matrix 自洽(requiredParams/optionalParams ⊆ properties)。白名单 scripts/ssot-allowlist.json 记录有意例外(runtime_assert.evidence_path 有意不暴露防任意路径写;ui.name 是 prop_name 历史兼容别名太泛不宜进 schema)。
- **P8-3 SLIM_CONFIG ui 条目移除**(P8-2 语义反转的必要补全):原 P2-11 曾有意删 ui schema 15 键(theme 系/tree/ops/geometry)约定"handler 仍从 args 读(additionalProperties)"——unknown 拒后该约定失效,不恢复 schema 则 theme/draw/prototype 参数上线即拒。schema 恢复 39 键,"schema 是参数 SSOT"防线成立前提是 schema 不撒谎。
- **测试**:test/p8-unit.test.ts 10 用例(状态机源码契约 GD-a..e:函数群/mtime+debounce/quiesce 记账/回滚+独立加载/重名+custom.list+env;TS-a/b:SLIM_CONFIG 移除+运行时 39 键;AUD-a/b/c:审计器真跑 exit 0+白名单结构+teeth——临时移除白名单项注入真漂移应 exit 1)+ test/p8-e2e.test.ts 4 用例(真起游戏:custom.list 诊断/运行中新增热加载/运行中坏契约回滚保旧实例/删除卸载)+ args-validator 4 新用例(unknown 拒+did-you-mean/低相似度无提示/无 properties 不查/数值)。
- **分发规则双副本同步**:bridge.md 新增「自定义命令热加载」段(状态机/quiesce/回滚/独立加载设计/引擎限制)+ core.md 新增「未知参数被拒 + did-you-mean」陷阱条(rule-templates.ts + .claude/rules,触发 bump 硬门禁——本批定版 0.32.19)。
- **独立审查与清偿(B-1/N-1/N-2/N-3 + 实现期漏改 6 测试,docs/reviews/2026-09-11-p8-batch.md)**:code-reviewer 判定 BLOCKING——**B-1(置信 85)unknown-param 拒绝破坏 godot_path per-call 覆盖**:ToolDispatcher 对所有工具读 args.godot_path(消费在 validateArgs 之前),schema 声明它的仅 10 个工具,约 19 个 headless 工具(particles/tilemap/signal/physics 等)传 godot_path 被硬拒;清偿:DISPATCHER_COMMON_KEYS 豁免集合(与审计器特判同源)+ 全链路回归用例 + core.md/模板补说明。N-1:p3-e2e/p8-e2e 补进 CI L2 白名单(L2 是白名单制,新 e2e 文件默认不进 CI,连续两批漏加)。N-2:ENV=0 冻结语义修复——discovery 分支仅启动扫描建新 slot + 删除分支不卸载(原实现新文件照样热加载、删除照样卸载,与"启动加载一次"文档不符)。N-3:重名冲突措辞修正——"先注册者赢"实为"路径字典序靠前者赢"(索引按 sort 顺序全量重建,五处同步)。另:全量测试抓出实现期漏改——删 SLIM_CONFIG ui 条目时漏改 module-loader-slim ×4 + ui-import-prototype ×2(旧断言反转 P8 语义),教训:语义反转类改动的破坏面探测须全量测试而非抽查。清偿后相关测试 179/179 + 门禁全绿。

## [0.32.18] - 2026-09-11

### Added — P7 批:语义观察层(gua 移植)——观察 profile + 可见性级联 + 字段投影 + UI role(42 仓尽调组3 #3/#4/#10,docs/reviews/2026-09-11-p7-batch.md)

- **P7-1 观察 profile + 可见性级联**(gua gua.cpp project_world_objects 移植):六读通道(find_nodes/get_node_properties/get_node_layout/monitor_start/watch_start/find_ui_elements)新增 `observation_profile` 参数("debug"|"player",默认 debug 直通现状)。player 档下:节点或任一祖先 meta `agent_exposure="private"` 或 `visible_to_player=false` → 整棵子树不可观察(get_node_properties/layout/monitor/watch 对其报 `Node not found`——存在性不泄露,报 404 而非 403)。两维度正交(private=对象级整藏 / visible_to_player=玩家不可见),与渲染 visible 无关(纯声明语义,对齐 gua visible_to_player 是 WorldObject 字段而非渲染状态)。**profile 门禁**:env `GODOT_MCP_BRIDGE_ALLOWED_PROFILES`(逗号分隔,默认仅 debug,_ready 读一次)——agent 不能自授权 player(模式切换不经 agent 通道,对齐 gua "profile 由 host 持有");未授权 -21,非法值 -20。
- **P7-2 字段级暴露策略五模式**(gua AgentFieldRule 移植,GD 单文件化 ~420 行):节点 meta `agent_field_rules`(Array,每条 `{path, mode, replacement?, quantum?}`,path 是输出 dict 键,支持 position.x/y/z 分量级;单节点上限 32 条,重复 path 后者覆盖)——`omit` 删键 / `redact` 数值归 0·bool 归 false·字符串换 "[redacted]" / `replace` 换声明替值 / `quantize` 数值按 quantum snapped(整键作用于 dict 值如 position 时**逐分量应用**)。**坏规则 fail-closed 降级为 redact**(mode 未知/replace 缺 replacement/quantum<=0——与 gua 注册时整 policy 拒不同,enhanced 无注册时机,读时逐条降级;写错规则不会被惩罚成裸暴露)。规则每次查询实时读取——游戏运行中改 meta 即时生效(战争迷雾类动态语义,gua "游戏逻辑更新迷雾内敌人 metadata,查询时引擎侧自动过滤")。
- **P7-3 monitor/watch 中途隐藏**:采样/事件在节点不可观察期间静默跳过(时间线缺格,不泄露),恢复可见自动续记;watch 事件 args 不做字段投影(信号参数是位置参数无字段名,设计偏离在代码注释与规则文档双声明——需要隐藏的信号把信号源藏进不可见子树)。
- **P7-4 near × 投影联动(P5 钩子清偿)**:P5 实现 near 时注释"position 被 OMIT 的对象不可测距防御依赖其投影层,enhanced 无投影层故不适用"——本批投影层落地后补上:player 档下 position 有任何字段规则的锚点被拒(-11,防距离差分反推隐藏/粗化坐标;gua 原文只禁 OMIT,enhanced 收紧为任何规则——quantize 距离同样泄精度)、候选节点被静默排除。
- **P7-5 UI role/label 语义树**(gua auto_adapter _control_role/_control_label 移植裁剪):find_ui_elements 输出新增 `role`(Button→button/CheckBox·CheckButton→checkbox/OptionButton→combobox/Slider·SpinBox→slider/LineEdit·TextEdit→textbox/Label→text/ProgressBar→progressbar/ItemList→list/TabContainer→tablist/ScrollContainer→scrollarea/其余→panel;子类先判防被基类吞)+ `label`(text 类控件取 text,OptionButton 取节点名——其 text 是选中项文本,与 value 语义混淆;容器类取节点名)。**全档位输出**(语义信息非隐私,Playwright a11y tree 的 Godot 手工版)。
- **设计偏离清单(相对 gua,代码注释+规则文档双声明)**:①投影层位置 C++ registry → GD bridge 单文件(分发模型约束);②坏规则处理 注册时整 policy 拒 → 读时逐条 fail-closed 降级;③path 体系 gua 世界模型 → enhanced 输出 dict 键;④QUANTIZE 精确有理数取模 → snapped()(尽调许可简化);⑤watch args 不投影(无字段名映射);⑥agent_allowed_actions 位掩码不移植(动作授权走既有三通道白名单,观察投影只管 read 通道);⑦editor 层(read_scene 等 headless 文件操作)不经此层——"防看穿"只覆盖 bridge 运行时通道。
- **实现过程探针/e2e 抓出两个真 bug(全修)**:①GDScript 4 的 `==` 不做 String↔bool 隐式转换,宽容解析 `v == false or str(v)...` 在 meta 值为字符串时是运行时 Invalid operands 错误(行为探针 PB-e 抓出,修为类型分支);②整键规则作用于 dict 值(position 是 {x,y})时 quantize/redact 把整个 dict 换 "[redacted]" 形状破坏(e2e OBS-b 抓出,修为逐分量应用)。
- **独立审查与清偿(B-1/I-1/N-1/N-4,docs/reviews/2026-09-11-p7-batch.md)**:code-reviewer 判定 BLOCKING——**B-1(置信 90)player 档"防看穿"承诺被四个未接线读出口击穿**:get_tree 整树序列化不过投影(直接违背"private 不进树")、wait_for_node 的 exists 与 wait_for_property 的 current 泄真值与存在性、playtest report 搭车直读真值、call_method 的 ALLOWED_METHODS 本质是只读方法集(get/get_children 可枚举 private 子树)——按入口名归"动作通道"是分类错误。清偿:四出口全接线(player 档下 get_tree 整枝剪除+info 投影/wait exists=false 且 match 基于投影值/report 与 step_until conditions 同语义投影/call_method 不可观察报 not found + get 返回值投影(含 await_completion 协程路径对称) + 结构枚举方法整组拒 -22)+ playtest.snapshot/restore 在 player 档拒 -23(保真语义冲突:投影快照 restore 会把投影值写回游戏)。**审查暴露的更深缺陷一并修**:请求级 profile 参数下 agent 可自降级 debug 绕过投影(env=debug,player 时 agent 全程传 debug 即白开)——env 升级三态语义(单值 `player`=host 强制档,全部观察请求被静默提升为 player,对齐 gua "profile 由 host 持有";多值=请求级可选;默认仅 debug),**I-1 同时消解**(单值 env 不再"关掉 debug",debug 请求照常服务只是按 player 投影)。N-1 replacement 补 _is_safe_value 检查(Object 型替值降级 redact);N-4 take_screenshot 补边界声明(玩家视角的诚实呈现)。清偿过程自修一处:**Edit 缩进事故**——conditions 段替换少一层缩进致 cond_error 块落出 for 循环体(su_entry 越界 Parse Error + 探针全体超时挂起,validate_scripts 曾 0 错因未跑完整编译;check:gdscript 与探针抓出)。
- **分发规则双副本同步**:bridge.md 新增「语义观察层」段(meta 约定/五模式/级联/门禁三态/边界诚实声明)+ find_nodes/find_ui_elements 表行更新(rule-templates.ts + .claude/rules,触发 bump 硬门禁——本批与 Unreleased 的 P6 一并定版 0.32.18)。
- **测试**:test/p7-unit.test.ts 20 用例(源码契约 12:投影核心/六接线点/B-1 四出口接线/强制档/near 联动/role 表顺序/门禁码;行为探针 8:真跑 Godot 直调内部函数——归一化矩阵/fail-closed 降级/三分支 redact/dict 投影/级联矩阵/门禁/强制档提升/role 表)+ test/p7-e2e.test.ts 13 用例(真起游戏:debug 直通对照/player 全投影/级联/存在性不泄露/near 联动/role 全档/monitor 采样投影+中途隐藏+恢复/watch 中途隐藏+恢复/门禁边界/get_tree 剪枝+投影/wait exists=false+match 投影值/call_method get 投影+枚举 -22+snapshot -23)+ p3-e2e 回归适配(fixture 新增 P7 节点改 NEAR 期望集,max_distance 500→400 避 SecretEnemy 距 500 浮点边界)。

### Security — P6 批:GDScript tokenizer 结构化扫描(Erodenn 移植)+ 非字面量 load 拦截(docs/reviews/2026-09-11-p6-batch.md)

- **P6-1 tokenizer 整文件移植**(src/core/gdscript-scanner.ts,432 行,来源 Erodenn-godot-mcp-runtime):手写状态机——剥注释/字符串内容、成员链合并(OS.execute → chain 数组,空白/换行容忍)、$节点路径/三引号/^StringName 归一化、行号列号;附带 classifyFirstArgument(括号深度感知的首参三分类 literal/none/nonliteral——"res://" + evil 不被前导字面量骗过)。
- **P6-2 scanGdscriptSandbox 新增 Phase 3:非字面量 load/preload/ResourceLoader.load 拦截**。正则(Phase 1)只拦字面量非 res:// 路径;变量/表达式形式(load(p)/load("res://" + evil))正则不可见(纯变量零特征)——tokenizer 深度分析补此漏网。取舍对齐 Erodenn Tier 1(动态资源路径=任意加载面,静态不可判指向);override 走既有 double opt-in。
- **架构判断(不移植三级 tier 的决策记录)**:enhanced 的安全架构是"工具级确认 + 内容级硬拦"——execute_gdscript actionRisk='process' 意味每次调用已 100% 经确认令牌 out-of-band gate(AI 不能自确认);Erodenn 的 Tier 2 elicit_required 是为"默认无确认"的 run_script 设计的分级确认,在 enhanced 是重复建设;且把文件写/网络类从硬拦降为"问了再跑"是安全回退(用户确认可能被注入内容欺骗,确认后的硬拦恰是纵深)。审计 sidecar 同理由不引入(决策已在确认令牌+审计日志双层中体现)。
- **runtime 工具族通道(executeGdscriptRuntime)**:Phase 3 上线首跑暴露结构性误伤——15 个 runtime 工具(animation/signal/physics/profiler/scene 等)的生成 code **内联模板函数**(主场景加载 load(_sp),路径来自 ProjectSettings,服务端固定行为)与 AI 代码同通道被拦(e2e 16 用例红)。新增 executeGdscriptRuntime 通道(opaque symbol,防 AI 经 execute_gdscript 伪造):**只跳 Phase 3,保留 Phase 1/2 全部防线**(与全豁免的 executeGdscriptTrusted 区分;AI 若经插值逃逸注入任意代码,OS.execute/ClassDB/网络类正则仍拦)。script.ts(execute_gdscript 工具本体,AI 自由代码唯一入口)刻意保持普通通道——Phase 3 主战场,测试锁定。
- **独立审查与清偿**:code-reviewer 判定 BLOCKING(1B+4N)同批清偿——B-1 workflow dev_loop 的 acceptance 断言经 wrapAssertionCode 间接消费同一模板头(load(_sp))走普通通道被 Phase 3 误拦(单测 mock executor 结构性假绿 + e2e 未覆盖 acceptance 路径,静默回归);修为 executeGdscriptRuntime(delivery 同款,AI 原文有 assertions.ts 前置全相位扫描兜底)+ e2e 补真链路回归锚。N-3/N-4 注释修正(适配点表述/scan-before-wrap 时序契约)。审查教训:共享 GD 模板常量内联 nonliteral load 后全部消费方被通道二分,须 grep 模板的全部**间接**消费方(wrap 函数调用点)而非只看 import 清单;修"误拦类"问题按威胁模型全量枚举消费方,而非按测试红灯清单打勾。
- **误报边界核实**:scanGdscriptSandbox 的输入源全部是 AI 提供代码(execute code/write content/override/assertion);内部脚本(godot_operations.gd 等大量合法 load(变量))直接 spawn 不经扫描——测试锁定该事实防未来误纳入。既有扫描消费方 5 文件 191 测试零回归。
- **测试**:test/p6-batch.test.ts 13 用例(tokenizer 五维行为/classify 三分类矩阵/非字面量 load 矩阵/注释字符串不误报/内部脚本隔离事实锚)。
- 本批未改工具清单/规则模板——按"默认不发版"定规不 bump 版本(AGENTS.md 2026-08-19),随下一触发批次定版(即本 0.32.18)。

## [0.32.17] - 2026-09-11

### Added — P5 发现批:discover 按需发现评分 + near 空间查询(上游报告 #9 / 42 仓尽调 P3 行动项,docs/reviews/2026-09-11-p5-batch.md)

- **P5-1 discover 按需发现评分**(上游 #9,token 经济学最后一块):manage_tools 新增 discover action(query 关键词,top 默认 8/上限 20)。默认 basic profile 只 ships 25 工具,未激活组(engine/blender/android/asset/debug 等)里的能力对 agent 不可见——discover 在**全量 45 工具**上按五维加权评分:工具名×5 > action 名×4 > 组名×3 > schema 属性描述×2 > 顶层描述×1(每词累计,大小写不敏感)。属性描述维度承重:P4 瘦身后 action 级能力词(如 game 的 take_screenshot)只在 method/params 属性描述里,顶层用概览词。返回匹配+所属组+激活态+激活指引(未激活组给 `manage_tools activate ["<group>"]` hint)——"能力可见性"不再依赖全量 ships(satellite discover_tools 模式的 enhanced 适配)。中文关键词可用(描述命中)。
- **P5-2 find_nodes 近邻查询**(gua near 移植简化):params 新增 near_node(锚点节点路径,须 Node2D/Node3D)+ max_distance(默认 1000,≥0)。只收与锚点同维度的节点(2D↔2D/3D↔3D,异维度不收),距离在 GD 侧计算(节点位置是引擎真相源),结果按距离升序(tie 按节点名,gua 同款确定性排序),每项含 distance 字段;锚点自身排除("找附近其他"语义)。gua 的"position 被 OMIT 不可测距"防御依赖其投影层,enhanced 无投影层故不适用(诚实边界)。错误码 -8/-9/-10(锚点不存在/锚点非 2D3D/max_distance 负)。
- **配套**:game params 描述补 find_nodes 形状(含 near);rules 双副本 find_nodes 行补 near 说明;P4 瘦身措辞微调(指针统一"见规则文档",schema 6023B——near 新能力的合理增长,per-tool warn 接受为提醒)。
- **独立审查与清偿**:code-reviewer 判定 BLOCKING(1B+1I+4N)全部同批清偿——B-1 near 树序截断先于距离排序(命中超 limit 时近距离节点静默丢失,小 fixture 结构性测不出;修为 near 模式大上限收集(≤500)+排序后截回 limit,e2e 补 limit=1 排序后截断行为锚);I-1 锚点非 2D3D(-9)/max_distance 负(-10)补 e2e 行为断言;N 系(用例重编号/always-allowed 组回填/审查文档落盘)。教训:排序型查询必须"先收集后截断",凡"遍历→排序"类工具必须有命中数>limit 的用例。
- **测试**:test/p5-batch.test.ts 7 用例(discover 评分排序/中文/多词/top 截断/basic 收窄 hint/空 query 拒绝 + near 契约)+ p3-e2e 追加 NEAR-e2e(Anchor/NearA/NearB/FarAway fixture,过滤+升序+锚点排除+负向,真引擎)。


## [0.32.16] - 2026-09-11

### Changed — P4 效率批:game 描述瘦身 + 失焦降速对抗 + inflight 孤儿记录(42 仓尽调 token 经济学/可靠性项,docs/reviews/2026-09-11-p4-batch.md)

- **P4-1 game 描述瘦身(token 维度实活,尽调 §二判断 3"描述膨胀单月翻倍")**:schema 占全量 82.5%,膨胀分两类——解释性长文本(可砍)与结构形状信息(不可砍)。game 三巨点压缩:顶层 description(1563→428B)/method 枚举(1594→~530B,逐方法解释删/枚举名保)/params 参数表(2089→~820B,紧凑形状保/长解释删);schema 8198→5800B,game 总 9761→6228B(-3.5KB)。被删细节(find_nodes root 限定/button 取值/button_mask)承接进 .claude/rules/godot-mcp-bridge.md 双副本(不进 tools/list 计费的完整知识载体)。**边界结论(尽调沉淀)**:scene.operations(2454B)/workflow.acceptance 等(2978B)是嵌套 JSON Schema 形状信息,每条子描述都是构造调用的必需输入,明确不砍——只砍解释性文本,不砍结构形状。
- **SLIM_CONFIG 口径勘误(审查 B-1)**:初版"零生效"标注把瘦身后产物值(~4.8KB)误当触发判断输入——阈值判断在变换前(原始 barrel schema ≥8000B 触发),slim 对 ui 实际生效(descHint 追加+theme 系属性移除已在产物中)。注释勘误为真实状态;game 无配置走人工压缩路线;机制保留作应急通道。
- **P4-2 失焦降速对抗(beckett 轻量版)**:编辑器失焦时 Godot 主循环 sleep 提到 unfocused 值(默认 ~10fps)——而失焦恰是 MCP 会话常态(人在聊天窗口)。editor 插件 websocket_server.gd:有 MCP 流量(新连接/入站消息)时 clamp 进程级 OS.low_processor_usage_mode_sleep_usec 至 60fps,空闲 10s 恢复原值,_exit_tree 兜底恢复。诚实边界:效果依赖 low_processor_usage_mode 开启(编辑器默认开);刻意不碰机器级 EditorSettings unfocused_*(NPGameDev 全量版才走,需备份+自愈全套)。
- **P4-3 inflight 孤儿记录(BuildersGate 取消经济学)**:工具调用开始/结束在 ~/.godot-mcp/inflight-<pid>.json 登记(≤16 条);进程被客户端 idle 杀后,**下一个 server 启动时**对残留文件 stderr 报丧("上个进程 pid X 死亡时仍在执行: tool A(开始于 Ns 前)")——把"付费任务扣费成功、文件落盘、agent 什么都没看到"的静默损失变成可诊断事件。报丧走 stderr(stdout 是 MCP transport);报过即清防重复;>24h 陈年残留静默清理;正常退出(GodotServer.close)清自己文件;GODOT_MCP_INFLIGHT_LOG=0 可关。
- **守卫收紧**:check-token-budget totalSum warn 90KB→95KB(瘦身后全量 ~94KB 转绿,回弹 >1KB 即警;error 120KB 硬线不动);engine/ui 两条 per-tool desc warn 保留(有意义的瘦身提醒)。
- **分发规则双副本同步**:find_nodes root 限定/button 取值/button_mask 承接条目进 rule-templates.ts + .claude/rules(触发 bump 硬门禁)。
- **独立审查与清偿**:code-reviewer 初判 BLOCKING(1B+3I+3N)全部同批清偿——B-1 SLIM_CONFIG"零生效"标注被证伪(把瘦身后产物值 4.8KB 误当触发判断输入;阈值判断在变换前,slim 对 ui 实际生效——registry 侧 descHint 存在为证),注释/测试/CHANGELOG/README 四处勘误;I-1 inflight 报丧前 process.kill(pid,0) 探活(防多 server 并存误报丧+误删活进程文件);I-2 docs/tools/*.md 再生成(此前停在 v0.32.8,help 输出静默漂移 8 版本);I-3 boost 期间回焦捕获 NOTIFICATION_APPLICATION_FOCUS_IN 放弃恢复(防"有焦编辑器 ~10fps")。另:全量中 ui-layout-integration 单跑 7/7 过(并行资源竞争 flaky,非本批)。附带修复 6+ 个描述契约测试对齐(仓库用测试固化工具描述=产品契约,瘦身前须 grep 全 test/ 搜锁定)。
- **测试**:新增 test/p4-batch.test.ts 10 用例——瘦身回归锚(schema<6500B 防回弹+指针/承接断言/守卫字面量)、失焦降速源码契约(clamp/恢复/退出三件+不碰 EditorSettings 负向)、inflight 真文件系统往返(登记/移除/孤儿报丧/陈年清理/满弃新/开关)。GD 行为由 check:gdscript 项目级完整编译兜底(errors=0)。


## [0.32.15] - 2026-09-11

### Added — P3 能力扩展批:弱网注入 + 真点击验证 + 项目本地命令 + profile 价格标签(42 仓深挖尽调 §7 P3 行动项,docs/reviews/2026-09-11-p3-batch.md)

- **P3-1 network_conditioner 弱网注入**:masteryee network_conditioner.gd 移植为 bridge inner class(_NetworkConditioner extends MultiplayerPeerExtension,Godot 4.1+ 内置可扩展类,无需编译 GDExtension)——装饰器包装真实 MultiplayerPeer,只改出向一条路径 _put_packet_script(静默丢包 randf<loss_pct / 延迟+jitter 进 pending 队列 16ms Timer flush),其余 ~20 虚方法直通。game 工具新增 network_conditioner action(op=set/clear/status)。边界诚实:无 peer(OfflineMultiplayerPeer)诚实报错不装空壳;clear 仅在 conditioner 仍是当前 peer 时恢复(游戏自行换过不覆盖)且幂等;只作用出向(host 侧装=影响 host→所有 client);无带宽限制;依赖 SceneMultiplayer 高阶 API。多人联机测试的独特能力(尽调组 2 唯一 P3 直抄项)。
- **P3-2 click_button real_event 真点击+信号计数验证**:默认路径保持 emit_signal("pressed")(不切换 button_pressed 状态——CheckBox/RadioButton"点击成功但没勾上"的根因,已在规则文档明示);传 real_event=true 时 press/release 两个 InputEventMouseButton 注入 viewport,走完整引擎输入管道(切换 button_pressed/button_group 互斥/focus),等 4 帧后响应带 signal_counts(pressed/toggled/button_down/button_up 计数,LuoxuanLove ClickSignalRecorder 移植)+verified(引擎真实发射信号的客观证据,而非只看自己 emit 的返回值)。延迟响应走 __CLICK_VERIFY__ 哨兵协程(同 call_method await_completion 模式)。
- **P3-3 custom_command 项目本地命令目录**:regiellis mcp_commands 模式移植——游戏开发者丢 .gd 进 res://mcp_commands/(get_commands() -> {"custom.xxx": Callable}),bridge _ready 宽容注册(load 失败/不可实例化/非 Node/无 get_commands/非 Dictionary 一律 push_warning 跳过,绝不破坏启动)。enhanced 约束:命令名强制 custom. 前缀(match 分发不可枚举内建名,前缀隔离天然零内建冲突;内建 match 优先=内建永远赢);信任边界与 GDA_CALLABLE 同哲学(声明面=游戏源码方责任,default deny)。game 工具新增 custom_command action(TS 侧前缀拦截 + validateBridgePath 对 custom.* 参数豁免——path 语义由游戏方定义);未声明命令 bridge 返 -32601。"扩展 API"的最佳轻量单点(无需 fork server)。
- **P3-4 manage_tools list_groups 价格标签**:beckett doctor 模式——响应新增 profiles 数组(每 profile 实测 tools/bytes/approxTokens),Buffer.byteLength 口径(非 string.length——中文描述场景 code unit 计数少报 2/3,beckett 实测 ~100 非 ASCII 少报 ~200B 同款坑)。切 profile 前先看价格(对照 BuildersGate 105k 基线,token 经济学落地)。
- **⚠️ 引擎坑实测(新发现,进 engine-quirks 双副本)**:ENetMultiplayerPeer.get_local_port() 在 Windows Godot 4.6.3 **阻塞挂死主循环**(探针二分定位:create_server(0) OK/set_multiplayer_peer 不挂,唯独 get_local_port() 挂;--script 与完整游戏均复现)——多人 e2e fixture 规避该调用。
- **分发规则双副本同步**:bridge 段(UI 表/新 action 段/坑段/P3 行为说明)+ engine-quirks 段(多人联机与弱网测试)进 rule-templates.ts + .claude/rules(触发 bump 硬门禁)。
- **独立审查与清偿**:code-reviewer 判定 SHIPPED WITH NITS(1I+4N,docs/reviews/2026-09-11-p3-batch.md)全部同批清偿——I-1 审查文档落盘;N-1/2/3 测试小修(基线断言/注释诚实化/断言去冗余);N-4(custom 无 BLOCKLIST)为设计权衡留档。全量测试另暴露 3 处本批引入回归并自修:validateBridgePath 的 take_screenshot 整体豁免误删 node_path 有意防御(既有 I-1 用例锁定,恢复只豁免 path 键,custom.* 才整体豁免);game-bridge.test.js action enum 期望清单补 2 新 action;manage-tools.test.ts vi.mock 补透传 PROFILES/getAllToolDefinitions。
- **测试**:新增 3 文件 27 用例——p3-unit(14:价格标签口径/Buffer 守卫/前缀拦截/validateBridgePath 豁免/源码契约 cmp-9 模式)、p3-network-behavior(5:真引擎探针,inner class 实例化+丢包/延迟/直通三分支+条件往返)、p3-e2e(8:真引擎端到端,emit vs real_event 双路径断言/ENet 生命周期/自定义命令正负用例)。fixture 新增 test/fixtures/p3-e2e(UI 按钮+ENet peer 建立/拆除+mcp_commands 示例,main.gd 用 GDA_CALLABLE 声明面,顺带验证与 P1 能力联动)。


## [0.32.14] - 2026-09-11

### Added — P2 批:函数级 profiling + report 搭车 + freeze 竞争上报 + 三个加固项(42 仓深挖尽调 §7 P2,docs/reviews/2026-09-11-p2-batch.md)

- **P2-1 函数级 profiling(debug 闭环硬缺口)**:整文件移植 Erodenn 的引擎原生 debugger profiler 流接收器(src/core/function-profiler.ts 884 行 + src/core/godot-variant.ts 172 行,经真引擎验证刻意不重写)——TS 侧解码 Variant 流([uint32 长度] 帧),产出**函数级 self/total 热点 + 调用数 + 峰值帧 + 最慢帧 top30 + 帧计时五项 avg/max**,等价编辑器 Profiler 面板。接线:run_project 新增 profiling 参数(spawn 前绑 127.0.0.1:0 随机端口传 --remote-debug,进程 close 自动清理;attach 会话无此通道为协议限制);profiler 工具新增 capture_functions action(一次调用采样窗口+排名,seconds/top/sort/capture_limit)。真引擎 e2e 实测通过(Godot 4.6.3,2 秒窗口端到端)。帧级分析(原 get_data)与函数级互补。
- **P2-2 report 结构化搭车**:playtest.step/step_until 可带 report=[{path,property}](≤16 条),响应自带终态读数——一次往返拿到"推进了什么",省掉紧随的观察调用(来源 satellite)。刻意结构化(不引入 Expression,对齐 step_until conditions 的 RCE 规避边界——Erodenn 规则表把 Expression 列 Tier 1 hard_block);属性过 _is_blocked_property;逐条失败带 error 不炸整调用;求值在推进完成后(读数反映最后一处理帧)。
- **P2-3 freeze 竞争上报**:游戏代码在 freeze 下 unpause(暂停菜单/自动取消暂停的过场)时 bridge re-assert 并计数;unfreeze 响应新增 frozen_for_ms/contested_reasserts(>0 = 游戏对抗过 freeze;每次 re-assert 可漏一帧,诚实报数而非假装密不透风;来源 satellite freeze_contested 变体)。
- **P2-4 bridge 资产清理加固**:uninstall 删 mcp_bridge.gd 时同步删 .uid 伴随文件(Godot 4.4+ 生成,漏删留孤儿);executeGdscript spawn 前置 orphan autoload 自愈——project.godot 残留 McpBridge 条目而脚本已删时(手删/卸载残留)每次 headless 操作都会因 autoload 加载失败而崩,现自动移除悬空条目并 warn(来源 Erodenn repairOrphaned;幂等)。
- **P2-5 CLI 退出码单一注册表**:src/core/exit-codes.ts(0 OK/1 OPERATION_FAILED/2 USAGE,shell 惯例值,类别粒度对齐 aigengame exit_codes.py);核心入口 14 处换常量;CI 静态扫描断言 src/cli 的 process.exit 字面值 ∈ 注册表(新码出表即红)。
- **P2-6 drive-relative 路径形态显式拒(实测抓到的洞)**:resolveWithinRoot 对 "C:foo" 形态此前静默回落项目根(path.resolve 丢弃 base 解析到盘符 cwd + safeRealPath 回落的组合歧义, BuildersGate padserver 实测形态的 enhanced 变体)——现显式拒绝,错误路径形态早暴露;补 UNC/绝对/.. 段/合法含点文件名回归锚用例。
- **分发规则双副本同步**:P2 control 层增强条目(report 搭车/freeze 竞争/函数级 profiling)进 rule-templates.ts + .claude/rules(触发 bump 硬门禁)。
- **独立审查与清偿**:code-reviewer 判定 BLOCKING(1B+2I+6N,docs/reviews/2026-09-11-p2-batch.md)全部同批清偿——B-1 step_until report 校验上移到开窗前(校验先于副作用,防 freeze 意外解除);I-1 freeze 重置仅新会话(重复 freeze 保留竞争历史,删死 clamp);I-2 repairOrphaned 键按写入方常量(MCPBridge=/autoload/MCPBridge=)+ 值校验防误删。清偿中额外自修两处:drive-relative 首版正则误伤项目内绝对路径(修为仅盘符后无分隔符形态,补放行锚);autoload 键大小写连环(MCPBridge 大写,实现与测试双双手误小写被审查修正暴露;纯函数断言拆独立文件隔离合载干扰)。
- **测试**:新增 4 文件 28 用例——profiler(Variant 编解码防御矩阵含真引擎 e2e)、report/freeze(契约 + 校验矩阵行为探针)、orphan 纯函数(逻辑/IO 分离重构)、exit/path/repair(注册表 + 静态扫描 + 路径逃逸矩阵 + IO 冒烟)。


## [0.32.13] - 2026-09-11

### Added — P1 安全批:输出侧防注入/自资产保护/心跳保活/声明制白名单(42 仓深挖尽调 §7 P1,docs/reviews/2026-09-11-p1-batch.md)

- **P1-1 nonce 信封输出侧防注入**:新增 `src/core/untrusted-wrap.ts`(NPGameDev untrusted.gd 的 TS 移植,26 行原版)——读回的项目内容包进带随机 nonce 的信封标签 + scrub 正则防嵌套伪造,让模型能区分"工具说的话"与"项目内容里的字"(六层防御全管"进",这是第一个管"出"的层;与"输出标记防伪造"正交)。接入 4 读通道 6 接点:read_script 两分支/read_scene 两处/get_debug_output/dev_loop 自由文本输出。软防御诚实边界:取决于客户端模型是否尊重标签语义。开关 `GODOT_MCP_UNTRUSTED_ENVELOPE=0` 可关(默认开)。
- **P1-2 插件自资产写保护(FileGuard)**:新增 `src/tools/shared/file-guard.ts`(来源 NPGameDev file_guard.gd)——write_script/edit_script/project_replace(整批原子)/scene commit/quick_scene 共 6 守卫点拒写 enhanced 自己分发到目标项目的插件资产(`addons/godot_mcp_server/**` + 任意位置的同名 `mcp_bridge.gd`(保守:非仅项目根,防路径形态变体)),防"被注入的脚本改写守卫再触发重载"的自毁攻击。尾段精确匹配防误伤同前缀兄弟目录;用户自己的 addons 照常可编辑;无 env 豁免(default-deny)。
- **P1 心跳保活(取消经济学)**:`src/core/progress.ts` 新增 `withToolHeartbeat`,dispatcher 主执行路径全量包裹——长工具每 20s 发 progress notification 心跳防客户端按 idle 上限(常见 1800s)杀掉"无响应"调用(server 侧照样跑完=钱照扣文件照写结果没处送,BuildersGate 夜班实测事故)。首个 tick 在 20s:短工具零开销零消息;无 progressToken(协议无法路由)时 no-op;message 明示 heartbeat 语义防误读为业务进度;finally 清 timer 防泄漏。
- **P1-4 GDA_CALLABLE per-node 声明白名单**:bridge `call_method` 白名单第三通道(来源 aigengame gda_harness.gd,同名约定生态互认)——游戏侧在节点脚本声明 `const GDA_CALLABLE := ["take_damage"]`,bridge 沿脚本基类链静态读 `get_script_constant_map()` 枚举(零项目代码执行,default deny)。粒度从 env 的进程级细化到节点级;信任边界=声明是声明者的断言(进游戏方 code review);EXTRA_METHODS_BLOCKLIST 统一升级为三通道硬底线(env/声明均不可越过)。分发规则双副本同步(rule-templates.ts + .claude/rules,触发 bump 硬门禁)。
- **测试**:新增 4 文件 18 用例——信封单元(5,含 scrub/nonce 随机性/开关)+ FileGuard 判定矩阵(4)+ 心跳 fake-timers(5,含异常清理)+ GDA_CALLABLE 行为探针(4,真跑 Godot 验证动态脚本声明枚举/default deny/StringName 项)。
- **独立审查与清偿**:code-reviewer 判定 BLOCKING(见 docs/reviews/2026-09-11-p1-batch.md),全部同批清偿——B-1 信封破坏 5 处既有裸 JSON.parse 断言(新增 `stripEnvelope` 消费方解包 helper 改造 script-csharp ×2 / runtime ×2 / scene-operations-mock ×3);I-1 BLOCKLIST 拒绝消息补 GDA 通道(env/声明双通道文案);I-2 stop_project 输出补信封(classifyOutput 两消费点防护对称);I-3 mcp_bridge.gd 守卫语义与三处文档对齐(任意位置同名,保守方向);N-1 正则 i 标志;N-2 scrub 放宽洗形似变体(非 hex nonce);N-4 两个 bridge e2e 的 install 前清 Windows EPERM 残留副本(防 install 幂等逻辑保留旧 bridge 测旧代码);N-6 心跳 timer unref;N-5 测试断言写实。补强测试:stripEnvelope 往返/形似变体 scrub/大小写矩阵/任意深度语义/stop_project 信封。
- **验证**:lint 零错误;build+bridge 三副本同步;check:gdscript errors=0;build-matrix v0.32.13;STRICT rules-sync 9 模板一致;全量 `npm test` 唯一失败为基线既有 stdin-close-shutdown(与本批零交集)。


## [0.32.12] - 2026-09-11

### Fixed — P0 修复批:42 仓深挖尽调三坑(2026-09-11,docs/research/2026-09-11-42仓深挖尽调.md §6 + docs/reviews/2026-09-11-p0-batch.md)

- **P0-1 callv 参数预检(修 null 误报成功坑)**:Godot `callv` 对无法转换的参数 push 引擎错误并返回 null,bridge 曾把 null 当"成功返回"误报(与 void 返回不可区分;aigengame 在 4.6.3 实测 String→int/Dictionary→Object/null→int/JSON array→Array[int] 全中)。修复:`_cmd_call_method` 在类型强转后、协程哨兵前加个数+类型双预检(`_call_args_precheck_error`,static func)——个数按 `required = declared - default_args` + vararg 判定;类型按 coerce 后可达性(严格转换表 `_BRIDGE_ARG_STRICT_CONVERSIONS`,JSON 6 源行来自 aigengame 真引擎 callv 一致性矩阵验证表 + INT 宽化行),typed Array/Dict 拒收,不可达组合显式拒绝 code -10;方法签名取不到的动态方法放行。同步 `game_write.call_method` 工具描述。
- **P0-2 .pc 日志 rotate race 防护(双 key)**:并发 Godot 实例共享 `user://logs`,桌面端 RotatedFileLogger 在 rotate 时竞争,abort 被伪装成 engine_crashed(最阴险的 flaky;aigengame conftest 实测)。修复必须**双 key**——base key 默认 false,但 `.pc` feature-tag override 桌面默认 true 且启动时获胜,只关 base 是 no-op。落地:7 个测试 fixture project.godot(test/e2e-scene + test/fixtures 6 个)+ `create_project` 模板 + `cli init` 模板;用户项目刻意不注入(边界留痕:多实例对用户项目的 race 为已知限制)。
- **P0-3 monitor 帧步长改游戏时间采样(satellite #378 同款坑)**:帧步长(interval_frames)在窗口期帧率变化时实际采样节奏漂移 2-4 倍(freeze 后 fps 读数过期尤甚)。修复:采样节奏锚定游戏时间(delta*1000 累计,含 time_scale;interval_ms = interval_frames * 1000/60,60fps 基准后兼容),`tree.paused`(游戏暂停/freeze)期间不计时不采样,长帧跨多个采样点只 resync 不 burst 补帧,monitor_stop 补采终态(带 paused 守卫);样本新增 `t_game_ms` 游戏时间戳。帧末采样(process_priority=1000)评估后**刻意不做**——bridge `_process` 还承担帧定时输入注入(send_input_sequence 的 at_frame 语义),整体移帧末会使输入生效帧偏移一位;决策注释留痕 `_ready`,统一迟一帧记为已知限制。
- **测试**:新增 4 文件 20 用例——callv 预检契约(5)+ **行为探针(8,真跑 Godot 直接调 static 预检,固化 type_name 级函数名事故防回归)** + 日志双 key 契约(2)+ monitor 游戏时间契约(5)。开发中踩坑:预检初版误用 `type_name()`(GDScript 无此全局函数,正确为 `type_string()`),契约测试与 `check:gdscript`(`--import` 不编译未被场景引用的 `src/scripts/*.gd`)双双漏报,由 error-capture 行为测试超时暴露——行为探针即为此盲区的补强。
- **分发规则双副本同步**:`src/tools/rule-templates.ts` 与 `.claude/rules/godot-mcp-bridge.md` 同步补 call_method 预检(-10)与 monitor 游戏时间调度条目(触发版本 bump 硬门禁,按 2026-08-20 N-C 裁决照常 bump;npm publish/tag 待用户指令)。
- **验证**:`npm run lint` 零错误;`npm run build`(bridge 三副本同步一致);`npm test` 6180 passed / 1 failed(唯一失败 `test/stdin-close-shutdown.test.ts` 为基线既有,git stash 对照证实);`npm run check:gdscript` errors=0;build-matrix + check:budget 0 error。独立 code-reviewer 审查:SHIPPED WITH NITS(2 Important 已于本批清偿,详见 docs/reviews/2026-09-11-p0-batch.md)。


### Fixed — MCP server outlives a client that dies without signaling it, leaking as an orphan process

- **Root cause**: `StdioServerTransport` (`@modelcontextprotocol/server`) only wires `'data'`/`'error'` listeners on `process.stdin`, never `'end'`. `index.ts` registers `SIGTERM`/`SIGINT` for a graceful shutdown, but nothing reacts to the client's write end of the pipe closing. A client that dies without signaling this child — crash, force-quit, a closed terminal window, not an unusual way for an editor/IDE integration to disappear — leaves the process running indefinitely: reparented to init once its original parent exits, stdio still open, nothing left to ever notice. Confirmed on a real machine: five such orphans accumulated from ordinary client session churn within a single afternoon, oldest still running unattended hours later.
