...
> **能力零丢失**:enhanced 继承 Coding-Solo 全部核心能力。工具结构从 13 个独立工具进化为 46 个 grouped tool(每个含多 action),**AI 读 schema 自动适配新结构**——你无需手改工具调用。
...
enhanced 继承 Coding-Solo 的全部核心能力,工具结构从 **13 个独立工具 → 46 个 grouped tool**(每个 tool 含多个 action):
