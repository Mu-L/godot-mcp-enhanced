...
- **下载信任链**:下载域名硬编码白名单(github.com / objects.githubusercontent.com / api.github.com,https 强制);版本 tag 白名单 `/^\d+\.\d+\.\d+-stable$/`;**SHA512 与二进制同 release 同信道**(`SHA512-SUMS.txt`,官方 releases 自带)——信任根 = GitHub 域名白名单,无跨信道信任假设(spec 未决项 1 实测结论:官方不提供独立 SHA256,提供同源 SHA512)。校验失败即删,不留半成品。
