...
- **死文件**：`test/helpers/` 现存 3 文件，grep 实测 `integration-setup.js` **0 引用**（死）、fixtures.js 7 引用、tool-context.js 9 引用。死文件检测器能立刻抓 integration-setup.js。
- **drift 信号**：src/gdscript-executor.ts 导出 18 个函数（executeGdscript / executeGdscriptTrusted / parseMcpMarkers / scanGdscriptSandbox / buildExecAuditEvent / …）。22 个 vi.mock 文件主要 mock executeGdscript(20)/parseMcpMarkers(7)/executeGdscriptTrusted(7)，均 ∈ src exports，**当前无 drift**。脚本守护未来 src 删/改名致 mock key 脱节（测试调 mock 值假绿）。
- **弱断言**：粗 grep toBeTruthy(1019)/toBeDefined(194)/not.toBeNull(134) ≈ 1347。toBeTruthy 占 76%。粗 grep 会含合理用法（result 存在检查）——**接受**，作"防恶化上限"非"消除目标"。

## 三检测器设计

### 检测器 1：死文件
- 扫 `test/**/*.{js,ts}`，排除 `*.test.*`（测试入口，由 vitest 跑，不查引用）。
- 对每个文件 basename（去 ext），grep 其在所有 `test/`+`src/`+`scripts/` 文件的 import/require 引用（`from ['"].*<basename>` 或 `require\(['"].*<basename>`）。
- **0 引用 → fail**（列出文件）。0 容忍。
- 已验：integration-setup.js 会被抓。

### 检测器 2：mock/src 导出 drift
- 扫所有 `vi.mock('<path>', () => ({ <keys> }))`（test/**/*.test.{js,ts}），正则提取 `<path>` + 工厂对象顶层 keys（`([a-zA-Z_][a-zA-Z0-9_]*):`）。
- 解析 `<path>` → 对应 `src/*.ts`（去 `../`/`build/`/`.js` 前后缀，定位 `src/<name>.ts`）。
- 提取 src 文件 export 集：grep `^export (async )?(function|const|class) NAME` + `^export \{ ... \}` named re-export。
- **drift = mock key ∉ 对应 src export 集**（mock 了 src 已删/改名的导出）→ fail（列出 mock key + src 文件）。0 容忍。
- **不抓**：src 新增导出 mock 未含（vi.mock 全替换，新导出 = undefined，测试调用会报错，非静默——非 drift 问题）；参数级签名 drift（需 TS 类型解析，超 :98 范畴，属 :97b）。
- 当前：3 个 mock key 都 ∈ src，无 drift，脚本 exit 0。
...
- TDD：先让脚本在当前代码上跑出预期（死文件抓 integration-setup / drift 0 / 弱断言 ≤1400）→ 删 integration-setup 后死文件 0 → 集成 package.json + ci.yml。
- 核实驱动：阈值/基线全实测（[[plan-baseline-verify-grep]][[verify-implementation-by-source]]）。

## Defer 清单

- :97b 23 vi.mock 补 failure 变体（drift 检测器的自然后续：drift 抓"导出名脱节"，:97b 抓"happy-path 绕过失败处理"，互补，:97b 另起）
...
- 脚本 `npm run check:test-quality` 在删 integration-setup 后 exit 0（死文件 0 / drift 0 / 弱断言 1347≤1400）。
...
- **一致性**：drift 定义（mock key ∉ src exports）与核实段（3 mock key ∈ src 18 exports，当前无 drift）一致；弱断言上限 1400 与基线 1347 一致。
