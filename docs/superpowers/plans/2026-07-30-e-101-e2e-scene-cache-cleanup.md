...
- **不动 CHANGELOG**（偏离 spec 的"补一行"——见下方偏差注）。grep CHANGELOG 全文证实同族 `:95`/`:97b`/`:98`/E-P1/E-P2 均未入 CHANGELOG → 本项目惯例＝内部测试改进不入 CHANGELOG（只记用户可见变更）。spec 那条是我未查先例的过度设定，plan 修正为不加。
...
> **对 spec 的偏差（plan 阶段新证据）**：spec「设计 → 不变项 / 出范围」段写"CHANGELOG 补一行（test-quality 段）"。plan 阶段 grep CHANGELOG 发现同族 5 个测试质量项无一入 CHANGELOG → 惯例为不入。故 plan 删除 CHANGELOG 步骤，只动测试文件。已在此标注供审查。
...
（编辑方式：用 MCP `edit_script` 的 `search_and_replace`，或内置 Edit 工具——本文件是 `.ts` 非 `.gd`，内置 Edit 可用。`old_string` = 原 4 行 beforeAll 体，`new_string` = 上面的完整块。）
...
```bash
cd D:/GitHub/godot-mcp-enhanced
git add test/e2e-p1-p5.test.ts
...
- 不入 CHANGELOG（同族 :95/:97b/:98 测试质量项惯例不入）；不进 defects.ts
- 基线 4277 passed | 24 skipped 实测无回归

Co-Authored-By: Claude <noreply@anthropic.com>
EOF
)"
```
Expected: `1 file changed, N insertions(+)`（仅 `test/e2e-p1-p5.test.ts`）。
...
- spec 验收 #4（.gitignore/ci.yml 无回归，diff 只动测试文件）→ Step 8 `git add` 仅 test 文件 ✅
- spec「CHANGELOG 补一行」→ **plan 偏差**（删除，见 File Structure 偏差注 + Global Constraints，同族先例证据）⚠️ 已标注供审查
- spec 出范围（NVIDIA/CI rm-rf 不动）→ Global Constraints ✅

**2. Placeholder scan：** 无 TBD/TODO/"适当处理"。所有 step 含具体命令/代码/期望输出。Step 3 编辑方式给了两条可选路径（MCP edit_script / 内置 Edit）但都有具体 old/new——可接受。无占位符。

**3. Type consistency：** `rmSync(resolve(E2E_DIR, '.godot'), { recursive: true, force: true })` 在 Step 3/4/commit message 全一致；`E2E_DIR` 一致。✅

**结论：** 覆盖完整，唯一偏离（CHANGELOG）已标注待审查。可执行。
