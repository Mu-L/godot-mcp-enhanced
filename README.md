...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v0.32.7** | 2026-08-19 | **scene_commit 新增 TileSet 碰撞配置两 op(竞品首创)**:`tileset_physics_layer_add`(加 physics layer,可选 layer/mask 位掩码,上报 layer_id)+ `tile_collision_set`(rect 全格四点等价编辑器按 F / polygon 自定义点集,可选 one_way;守卫链 atlas source/has_tile/physics_layer 越界/TileData 结构化报错)。被改 .tres 走 tmp+rename 原子写,纯节点 commit 生成物零变化;安全分层 res:// 浅校验 + 已存在 .tres 的 realpath 纵深(%2e%2e 绕过负向测试)。端到端 Godot 4.6.3 实测并修正两处文档偏差(PackedVector2Array 只收 Array;has_tile 实收 1 参)。43 工具/241 action。 |
