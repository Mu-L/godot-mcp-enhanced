...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v0.32.7** | 2026-08-19 | **scene_commit 新增 TileSet 资源层配置 9 op(竞品首创,physics/navigation/custom data 三层全可编程)**:碰撞族(tileset_physics_layer_add/set/remove + tile_collision_set rect/polygon/one_way + tile_collision_clear)、导航族(tileset_navigation_layer_add + tile_navigation_set,配 nav 工具形成铺瓦片→配导航→寻路闭环;引擎侧对象级 NavigationPolygon 生成器已适配)、自定义数据族(tileset_custom_data_layer_add name+type 六类型 + tile_custom_data_set per-tile 写值)。per-tile 守卫链四 op 共用(资源/atlas source/has_tile/越界/TileData 全结构化报错);被改 .tres 去重 tmp+rename 原子写,纯节点 commit 生成物零变化;安全分层 res:// 浅校验 + 已存在 .tres realpath 纵深(%2e%2e 负向测试含扩展批接线判别)。端到端 Godot 4.6.3:11 op 全链 + 重载断言 12 项 + 负向 3 类,证据归档 docs/reviews/(COMMIT_RESULT 原文);实测修正两处文档偏差(PackedVector2Array 只收 Array;has_tile 实收 1 参)。43 工具/241 action。 |
