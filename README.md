...
工具层:45 个 MCP 工具(merged,共 248 个 action;完整清单见 [capability-matrix](docs/capability-matrix.md))覆盖场景/脚本/UI/动画/物理/粒子/导航/音频/测试/导出/3D 参数化资产(asset:11 shape + 路径阵列 + batch 原子 undo),三层架构
...
| 工具数 | **45** ([matrix](docs/capability-matrix.md)) | 175 [^p1] | ~30 [^p1] | 13 [^p1] | 386 [^p4] |
...
> **⚠️ 诚实披露 update-checker 外传点**:本仓库每次 MCP server 启动时,`src/core/update-checker.ts` 的 `fetch(REGISTRY_URL)` 会**被动 fetch** `https://registry.npmjs.org/godot-mcp-enhanced/latest`(24h 缓存)。此行为与遥测无关但涉及「数据离开本机」。**v0.25.7 起支持 `GODOT_MCP_UPDATE_CHECK=false`(或 `0`/`no`/`off`,大小写不敏感)关闭启动外传**;`self_update` check action 经 `force:true` 短路此门控,且 risk='read' 不经确认令牌,**AI 可自主调用触发外传**(IP/UA 泄漏 npmjs.org)。严格零外传需防火墙或 readOnly 模式拒整工具。CLI 下载链(`install`/`web` 的 GitHub releases 下载,用户主动触发)是另一独立出网点,详见 [`docs/telemetry.md`](docs/telemetry.md#-诚实披露既有的非遥测外传点)。
>
> **代理环境变量(实测口径)**:Node 原生 fetch(undici)**默认不读** `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` 环境变量(Node ≥24 可设 `NODE_USE_ENV_PROXY=1` 启用)。实测设必拒代理端口后 fetch registry.npmjs.org 仍直连 200。企业代理环境下更新检查**实际直连**(可能被防火墙静默拦截);`NO_PROXY` **不是**零外传的有效手段。详见 [`docs/telemetry.md`](docs/telemetry.md) 代理环境变量节。
>
...
> 共 45 个 MCP 工具(merged tool definition,共 248 个 action),以下按 action 逐项展开全部操作;权威清单见 [capability-matrix](docs/capability-matrix.md)。
>
...
| `uid_set` | 写 `.uid` 文件：指定 uid / 按路径确定性生成（与编辑器一致）/ `fix_missing` 批量修复缺失 |
...
| `translation_register` | 把 `.translation`/`.po` 资源注册进 `project.godot` 的 `internationalization/locale/translations`（`remove=true` 反向移除） |
...
> ✅ 协议层实测通过（45 工具全发现、inputSchema 完整、无 integer 参数兼容风险）；⚠️ Warp GUI 端到端待补（本机未装 Warp）。完整步骤、兼容性核对表、env / `working_directory` 说明见 [使用指南-Warp](docs/使用指南-Warp.md)。
...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v0.32.12** | 2026-09-01 | **竞品启发加固批(monitor 可解释性/GODOT_PATH 目录报错/原子写收口)**:①bridge monitor 输出可解释性(对标 satelliteoflove)——修复 monitor_start 返回谎报监控范围(properties 原始请求列表→实际监控列表,被安全过滤属性逐个点名进 dropped_blocked),stop/poll 附数值极值摘要 summary(min/max 及发生帧/时刻,仅数值属性;error 样本跳过,同值极值首现),窗口自述 max_samples/interval_frames;bridge 行协议 UTF-8 跨 chunk 免疫性质契约固化(对标 blender-mcp 3100b36:字节缓冲+\n 切行+整行解码+无效 UTF-8 断连)。②GODOT_PATH 指向目录显性报错(对标 godot-ai 69ba29f):不再静默 fallback 到 registry/scoop 掩盖配置错误,错误消息 PII-safe;validateGodotBinary 统一拒绝目录候选。③共享原子写上移 src/core/fs-atomic.ts(对标官方 servers 562feeb/642a911):三份重复实现合并(mode 保持+随机 tmp+Windows 锁定降级),薄委托保 13 消费方零改动;接入 save_scene 写回 .tscn/edit_script 三路径/quick_scene,存量七处(scene-instance/translation-ops/game-bridge/overrides)同日收口清零。④godot-finder tried 死代码清除;规则模板 monitor 行同步(dropped_blocked/summary,双副本)。验证:lint/build 0 错、全量 6165 passed、check:gdscript 0 错、headless 真跑摘要算法 7/7;第三方审查 SHIPPED WITH NITS 0 Blocking。45 工具/248 action。版本号由规则模板硬门禁强制 bump(N-C 条款,npm publish 待用户)。 |
