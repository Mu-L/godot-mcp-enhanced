...
describe('navigation 生成器: set_owner 判空守卫(防回归)', () => {
  ...
  it('genCreateRegionScript: set_owner 经 _owner 判空', () => {
    ...
    expect(script).toContain('var _owner: Node = _mcp_get_root()');
    expect(script).toContain('if _owner != null:');
    expect(script).toContain('_nav.set_owner(_owner)');
  ...
  it('genCreateAgentScript: set_owner 经 _owner 判空', () => {
    ...
    expect(script).toContain('var _owner: Node = _mcp_get_root()');
    expect(script).toContain('if _owner != null:');
    expect(script).toContain('_agent.set_owner(_owner)');
    expect(script).not.toContain('set_owner(_mcp_get_root())');
  });
  ...
  it('genCreateLinkScript: set_owner 经 _owner 判空', () => {
    ...
    expect(script).toContain('var _owner: Node = _mcp_get_root()');
    expect(script).toContain('if _owner != null:');
    expect(script).toContain('_link.set_owner(_owner)');
    expect(script).not.toContain('set_owner(_mcp_get_root())');
  });
});
