...
describe('N2 nav status 派生（反硬编码）', () => {
  it('N2: bake status derived from success (not hardcoded)', () => {
    ...
    // 正向：async 分支也按 success 派生（GD-R1: 与 success 同源,消除矛盾语义）
    expect(gd).toMatch(/"status":\s*"bake_completed"\s+if\s+success\s+else\s+"bake_timeout"/);
