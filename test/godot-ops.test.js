...
describe('TYPE_WHITELIST', () => {
  it('contains Node3D', () => { expect(TYPE_WHITELIST).toContain('Node3D'); });
  it('contains MeshInstance3D', () => { expect(TYPE_WHITELIST).toContain('MeshInstance3D'); });
  it('contains Camera3D', () => { expect(TYPE_WHITELIST).toContain('Camera3D'); });
  it('contains RigidBody3D', () => { expect(TYPE_WHITELIST).toContain('RigidBody3D'); });
...
describe('genNavQueryScript', () => {
  it('contains NavigationServer3D.map_get_path', () => {
    ...
    expect(script).toContain('NavigationServer3D.map_get_path');
    expect(script).toContain('Vector3(0, 0, 0)');
    expect(script).toContain('Vector3(10, 0, 10)');
  });
  it('includes region lookup when provided', () => {
    const script = genNavQueryScript({x:0,y:0,z:0}, {x:10,y:0,z:10}, '/root/NavRegion');
    expect(script).toContain('NavigationRegion3D');
    expect(script).toContain('/root/NavRegion');
  });
  it('includes fallback maps logic', () => {
    const script = genNavQueryScript({x:0,y:0,z:0}, {x:10,y:0,z:10});
    expect(script).toContain('get_maps');
