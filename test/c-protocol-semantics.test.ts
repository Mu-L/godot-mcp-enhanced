...
describe('C3/C9 协议返回值语义', () => {
  // C4 (nav bake accurate vertices_count + coroutine await) deferred —— 同步 dispatch 链路
  // (command_handler.gd:144 + websocket_server.gd:350-351) 不支持 coroutine handler（架构阻塞）。
  // 详见 nav_commands.gd handle_nav_create_region 头注释 + defects.ts nav-bake-in-undo-action deferral。
